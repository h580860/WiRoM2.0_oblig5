...
def moose_main():
    ...
    while robot.step(timestep) != -1:
        ...
        # print(f'(moose3) step number {step_count}')
        step_count += 1
...
def test_receive_routing_message():
    ...
    channel.queue_bind(exchange='routing_exchange', queue=queue_name, routing_key="moose3_queue")

    print("Moose3 ready to receive routed messages")
...
def test_receive_location():
    ...
    channel.queue_bind(exchange='location_exchange', queue=queue_name, routing_key="moose3_location_queue")

    print("Moose3 ready to receive locations")
...
def execute_simpleactions_callback(ch, method, properties, body):
    ...
    while simpleactions:
        ...
        print("(moose3) Executing simpleaction " + sim_act)
        eval(sim_act)

def receive_location_callback(ch, method, properties, body):
    global location
    print("(moose3) received locations")
